...
router.get('/', authMiddleware, async (req, res) => {
  const { from, to, churrascaria_id, produto_id } = req.query;
  const params = [];
  let query = `
    SELECT pe.id, pe.data, pe.funcionario, pe.quantidade, pe.motivo, pe.created_at,
           p.name as produto_name, p.unit as produto_unit,
  ...
  if (produto_id)      { params.push(produto_id);      query += ` AND pe.produto_id = $${params.length}`; }
...
router.post('/', authMiddleware, async (req, res) => {
  const { data, funcionario, churrascaria_id, produto_id, quantidade, motivo } = req.body;
  if (!data || !funcionario?.trim() || !produto_id || !quantidade) {
    return res.status(400).json({ error: 'Data, funcionário, produto e quantidade são obrigatórios' });
  }
  try {
    const { rows } = await pool.query(
      `INSERT INTO perdas (data, funcionario, churrascaria_id, produto_id, quantidade, motivo, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
      [data, funcionario.trim(), churrascaria_id || null, produto_id, quantidade, motivo?.trim() || null, req.user.id]
    );
